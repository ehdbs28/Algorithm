...
스택으로 풀면 이미 있는 기능을 그대로 사용하는 느낌이라 배열로 풀어보기로 하였다.
...
> #### [문제풀이](https://github.com/ehdbs28/Algorithm/blob/main/BAEKJOON/Data_Structure/22942_Data%20Checker.cpp)

이 문제는 푸는데 굉장히 굉장히 어려움이 든 문제였지만 선생님의 도움으로 풀 수 있었다 !

우선 이 문제는 간단하게 **수직선에 그려져 있는 모든 원이 하나라도 겹쳐있는지 확인하는 문제이다.**

처음에는 두가지 방법을 사용해서 문제에 접근하였다.
```c
1. 원을 순서대로 스택에 담아 하나씩 pop 하며 비교하는 방법
2. 모든 원의 값을 반복문을 돌리면서 비교하는 방법
```
하지만 첫번째 방법으로 할 경우 값을 바로 전에 값 하고만 비교하기 때문에 문제가 되었고, 두번째 방법은 시간복잡도가 너무 늘어나버려 시간초과로 해결할 수 없었다.

새로운 접근법을 찾아내었다.

생각해보니 수직선에 앞에 있는 순서대로 원을 정렬한 후 반복을 돌리면 원이 앞에 있는 경우가 무시되니
```
...
오랜만에 길게 생각하면서 풀어야하는 어려운 문제를 접하니 조금 어지러웠지만 풀고난 후 재미있었다.